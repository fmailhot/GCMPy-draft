...
import numpy as np
import pandas as pd
from GCMPy_utils import HzToBark, check_accuracy, overall_acc, get_testset
...
def choose(probsdict,test,cats,runnerup=False,fc=None):
    ...
    for cat in cats:
        ...
        # Get the two highest probs for each cat type
        best2 = pr[cat]['probability'].nlargest(n=2).reset_index(drop=True)

        # Match the prob to the category
        choiceprob = best2[0]
...
def get_activ(activation,x,y,cat):
    """ 
...
def multi_cat(testset, cloud, cats, dims, c=25, N=1, biascat=None, catbias=None, rescat=None,
              ncyc= None, exclude_self=True, alsoexclude=None, runnerup=False, fc=None):
    '''
...
def error_func(x, testset, cloud, dimslist, cat):
    ''' 
    ...
    choices=multi_cat(cloud,testset,catlist,dims=dimsdict,c=c)
    accuracy=check_accuracy(choices,catlist)
...
if __name__ == '__main__':
    ...
    catslist=['vowel','type']
    # categorize testset
    choices = multi_cat(test, pbbark, catslist, dimsvals, cval, exclude_self=True,
                       alsoexclude=None, N=1, runnerup=False)
    # check accuracy
    acc = check_accuracy(choices,catslist)
    print("overall accuracy: " + str(overall_acc(acc,'vowel')))
